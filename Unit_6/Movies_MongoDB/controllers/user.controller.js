    ...
    //  res.send("Connected, hit signup endpoint!");
     ...
     try {
      ...
      const user = new User({
        ...
        // We want tot pass in the password provided by the user, salt it 13 times, and add to db
...
router.post("/login", async (req, res) => {
  //res.send(req.body.email); // Used to test the endpoint is working

  try{
    //1. Capture data provided by user (request body), use object destructuring (easy to grab key/values)
    ...
    //3. If email exists, consider if email matches (decrypt).
      //. compare(method) from bcrypt take in 2params(password from req.body, hashed password from DB)
