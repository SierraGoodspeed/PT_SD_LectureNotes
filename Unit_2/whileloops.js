...
let city = [              //<--- this is an array
  ...
  let pos = [0];
    ...
    console.log (`Spider-Man lands is the ${structure}!`);



/* 
    ? Do While Loop: also loops through a block of code while a specified conditions is true BUT...
...
do {
    ...
    console.log(
        `acct${acctNum} is currently inactive: set to ${accounts[`acct${count}`]}`
      );
        //Activating message & set acct to true
        console.log ("Activating....");
        account [`acct${acctNum}`] = true;

        //confirmation message
        console.log(
            `acct${acctNum} is now active: set to ${accounts[`acct${count}`]}`
            )
            // After account has been activated, add 1 to acct# to move on to next account
            acctNum++;
    
} while (account [`acct ${acctNum}`] == false);
