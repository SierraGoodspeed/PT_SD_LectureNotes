...
router.get('/:id', (req, res) => {
    ...
    // Detail a object with a key of "id" (our parameter name) & and the value within the URL we passed to it
   ...
   try {
    // Use object destructuring so id is equal to our /#, /:id, id = 1, id = 2, etc.
        let {id} = req.params;
        // Setting the variable results to equal whatever item in the DB has the same id as the variable ids value/ .filter() method checks each item in the array till it finds the one who's id matches.
...
router.post("/", (req, res) => {
    try {
        ...
        fs.readFile("./helpers/db.json", (err,data) => {
            ...
            // Parses the JSON file and provides and array of our objects back so we can manipulate it
...
/* 
    ...
    - check if id matches param(if else of other conditionals check)
    ...
    * stretch goal: update just the data without modifying the ID
    ...
    []  Create logic that uses an array method to find and math the param to a db item: forEach
...
try{
        ...
        fs.readFile("./helpers/db.json", (err, data) => {
            ...
            // Using forEach method to basically find and then replace the item that matches the param id

            db.forEach((e, i) => {
                console.log(e, i );
            // If the object/items's id match the param, we do something
            if(e.id === id) {
            ...
            db[i] = todo;
            // Reassign/assign result var the todo var as well
            result = todo;
            ...
            // whatever your testing ? : (ths runs if true, 200) : (this runs if false, 404/500)
...
// TODO: Delete a task
/* 
    - pass ID as a param, Numberfied it: const id = Number(req.params.id);
    ...
    try {
        ...
        fs.readFile("./helpers/db.json", (err,data) => {
            ...
            const filteredDB = db.filter((e)=> {
                // we want to check for the id... and return only what does'nt match 
                if (e.id !== id) {
                return e;
                }
            }); 
