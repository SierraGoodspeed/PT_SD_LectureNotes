...
const stateAbbreviation = {
  ...
  // Create a findIt function that will use whatever object(dict) and key term(search) give and return the value result if found
    ...
    let statesInUS = findIt(stateAbbreviation, "vermont");
    console.log(statesInUS);
...
const howIsChild = (state) => {
    if (state == "Hungry") {
        return "THE KIDS WANT FOOD, WHERE'S THE SNACKS!?!";
    }else if (state == "Sad"){
...
const kidFeelsTable = {
    Hungry: "THE KIDS WANT FOOD, WHERE'S THE SNACKS!?!",
...
/* 
    * State Machines:
    ...
    State: is the set of all variables in a program and their values at ant point in time
...
function enterState(newState) {
    ...
    if (validTransitions.includes(newState)) {
        // Allow our state to change if true next state
